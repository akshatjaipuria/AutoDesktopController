...
GATEWAY_V9_DIR = Path(__file__).resolve().parents[1] / "llm_gateway"
...
def ensure_gateway() -> None:
    ...
    if not GATEWAY_V9_DIR.exists():
        raise RuntimeError(
            ...
            "Build llm_gateway (Session 9 prerequisite) before running DesktopController code."
        )
    print(f"[gateway] launching llm_gateway from {GATEWAY_V9_DIR}")
    subprocess.Popen(
        ["uv", "run", "main.py"],
...
def embed(text: str, task_type: str = "retrieval_document") -> dict:
    ...
    if LLM is None:
        raise RuntimeError(
            "Gateway V9 client unavailable. Confirm llm_gateway/client.py exists."
        )
